...
ColorString.colorValue = function colorValue (colors, g_table, isRGBW) {
    // colors are assumed to be an array of [r, g, b] bytes with optional w value
    // if rgbw format is assigned in color_order
    ...
    if (colors.length > 3 && isRGBW){
        // if we have RGBW set to true and we have 3 values in the array
        // assign the 4th value and return colorValue rgbw values
        colors[3] = g_table[colors[3]];
        return ((colors[0] << 24) + (colors[1] << 16) + (colors[2] << 8) + (colors[3]));
    } else {
        // if we have RGBW set to false return colorValue to reflect rgb values
    ...
    FIRMATA: {
        initialize: {
            value: function(opts) {
                ...
                isRGBW = color_order === COLOR_ORDER.RGBW;
...
Strip.prototype.colour = Strip.prototype.color = function(color) {
    ...
    if (stripcolor != null) {
        ...
        // TODO (CF) Look into stripcolor here to find the green value - double check what colorVlaue returns
        this.strip_color(ColorString.colorValue(stripcolor, strip.gtable, isRGBW));
...
Pixel.prototype.colour = Pixel.prototype.color = function(color, opts) {
    ...
    if (pixelcolor != null) {
        ...
        if ((isRGBW && pixelcolor.value.length === 5) || (!isRGBW && pixelcolor.value.length === 4)) {
            pixelcolor.value.pop();
        }
        // TODO (CF) Is this being used?
        pixel.color.rgb = pixelcolor.value;

        color = ColorString.colorValue(pixelcolor.value, pixel.parent.gtable, isRGBW);
        if (sendmsg) {
